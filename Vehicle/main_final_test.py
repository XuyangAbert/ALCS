...
def Input():
    sample = pd.read_csv('vehicle.csv',header=None)
    [N,L] = np.shape(sample)
...
#    stdData = 1
    ...
    radius = numpy.linalg.norm((maxLimit-minLimit)) * 0.1 # 0.2
...
#    new_fetchY = label_Fetch[reverse_idx[:round(len(FetchIndex)*0.85)]]
    ...
    X_train, X_test, y_train, y_test = train_test_split(AccSample, label, test_size=0.90, random_state=42)
