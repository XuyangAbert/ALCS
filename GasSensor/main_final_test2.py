...
def Input():
    sample = pd.read_csv('GasSensor.csv',header=None)
    [N,L] = np.shape(sample)
...
if __name__ == '__main__':
    ...
    for it in range(len(label_ratiovalues)):
        ...
        dense_idx = np.where(cluster_radius<=delta)[0]
        ...
        for i in range(np.shape(P)[0]):
            ...
            if i in dense_idx:
                ...
                FetchIndex = np.append(FetchIndex,fet1)
                FetchIndex = np.append(FetchIndex,fet2)  
                UnlabeledIndex = np.append(UnlabeledIndex,tempcluster[0][sortIndex1[round(fetchSize*0.5):round(len(d1))]])
            else:
