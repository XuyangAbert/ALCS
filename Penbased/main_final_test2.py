...
def Input():
    sample = pd.read_csv('penbased.csv',header=None)
    [N,L] = np.shape(sample)
...
#    stdData = 1
    ...
    radius = numpy.linalg.norm((maxLimit-minLimit)) * 0.2 # 0.2
...
#        fet2 = tempcluster[0][fil_index[sortIndex2[:round(fetchSize*0.2)]]]
        fet2 = tempcluster[0][candidate_fet2[sortIndex3[:round(fetchSize*0.2)]]]
        fet2 = fet2.astype(int)
...
#    new_fetchY = label_Fetch[reverse_idx[:round(len(FetchIndex)*0.85)]]
    ...
    X_train, X_test, y_train, y_test = train_test_split(AccSample, label, test_size=0.92, random_state=42)
...
#    clf6 = LR()
    ...
    clf1 = clf1.fit(sample_Fetch,label[FetchIndex])
    clf2 = clf2.fit(sample_Fetch,label[FetchIndex])
    clf3 = clf3.fit(new_fetchX,new_fetchY)
    clf4 = clf4.fit(new_fetchX,new_fetchY)
    ...
    pred_label4 = clf4.predict(sample_Unlabeled)
    pred_label5 = clf5.predict(X_test)
    pred_label6 = clf6.predict(X_test)
    
#    label1_match = np.where(pred_label1== label_Unlabeled)
#    label2_match = np.where(pred_label2== label_Unlabeled)
#    label3_match = np.where(pred_label3== label_Unlabeled)
#    label4_match = np.where(pred_label4== label_Unlabeled)
#    label5_match = np.where(pred_label5== y_test)
#    label6_match = np.where(pred_label6== y_test)
    
    Acc1 = accuracy_score(label_Unlabeled,pred_label1)
    Acc2 = accuracy_score(label_Unlabeled,pred_label2)
    Acc3 = accuracy_score(label_Unlabeled,pred_label3)
    Acc4 = accuracy_score(label_Unlabeled,pred_label4)
    Acc5 = accuracy_score(y_test,pred_label5)
    Acc6 = accuracy_score(y_test,pred_label6)
    ...
    F1_4 = f1_score(label_Unlabeled,pred_label4,average='macro')
    F1_5 = f1_score(y_test,pred_label5,average='macro')
    F1_6 = f1_score(y_test,pred_label6,average='macro')
    
    print('Accuracy:', Acc1, Acc2, Acc3, Acc4, Acc5, Acc6)
    
    print('F-measures:', F1_1, F1_2, F1_3, F1_4, F1_5, F1_6)
