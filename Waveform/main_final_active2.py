...
def Input():
    sample = pd.read_csv('waveform.csv',header=None)
    [N,L] = np.shape(sample)
...
#    stdData = 1
    ...
    radius = numpy.linalg.norm((maxLimit-minLimit)) * 0.2
...
def DiversityFetch2(candidate_fet2, current, priority2, interd1, dth, fetchsize):
    ...
    for i in range(num_border):
        ...
        neighbordist = chunked_dist[top_idx][:]
        neighboridx = np.where(neighbordist <= dth)[0]
        priority2[top_idx] = priority2[top_idx] / (1 + 300*np.sum(priority2[neighboridx]))  
